...
public class FileManager {
	...
	private static final String imagePath = rootPath+"/clientImage/"; 
	
	//외부에서 파일 접근 경로를 제한
	public static enum PATH{ ROOT, CLIENTIMAGE };
	...
	public static boolean createDir(PATH path, String dirName) {
		...
		try {
			...
			System.out.println("[클라이언트] 디렉터리 생성 : "+dirPath.getFileName());
			isCreated=true;			
		}catch(FileAlreadyExistsException e) {
			System.out.println("[클라이언트] 디렉터리가 이미 존재함");
			isCreated=true;
		}catch(NoSuchFileException e) {
			System.out.println("[클라이언트] 파일을 찾지 못함");
			isCreated=false;
		}catch(IOException e) {
	...
	public static boolean deleteDir(PATH path, String dirName, boolean recur) {
		...
		try {
			...
			if(Files.exists(dirPath)&&Files.isDirectory(dirPath)) {
				...
				System.out.println("[클라이언트] 디렉터리 삭제 : "+dirPath.getFileName());
			}else {
				System.out.println("[클라이언트] 없거나 디렉터리가 아님");
			}
			isDeleted=true;
		}catch(IOException e) {
	...
	public static boolean writeFile(PATH path, String subPath, byte[] bytesFile) {
		...
		try {
			...
			System.out.println("[클라이언트] 파일 생성 : "+filePath.getFileName());
			isWrited = true;
		}catch(IOException e) {
	...
	public static boolean deleteFile(PATH path, String subPath) {
		...
		try {
			...
			isDeleted = true;
			System.out.println("[클라이언트] 파일 삭제 : "+filePath.getFileName());
		}catch(IOException e) {
